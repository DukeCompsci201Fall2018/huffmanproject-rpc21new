...
//		}
	...
	private void writeCompressedBits(String[] codings, BitInputStream in, BitOutputStream out) {
		while (true) {
			...
			if (myDebugLevel >= DEBUG_HIGH)
				System.out.printf("%d wrote %d for %d bits\n", val, Integer.parseInt(code, 2), code.length());
		}

		String code = codings[PSEUDO_EOF];
		out.writeBits(code.length(), Integer.parseInt(code, 2));
		if (myDebugLevel >= DEBUG_HIGH)
			System.out.printf("%d wrote %d for %d bits\n", PSEUDO_EOF, Integer.parseInt(code, 2), code.length());
//		String code = codings[PSEUDO_EOF];
//		out.writeBits(code.length(), Integer.parseInt(code, 2));
...
//			return;
	...
	public void decompress(BitInputStream in, BitOutputStream out) {
		...
		if (myDebugLevel >= DEBUG_HIGH) System.out.println("Printing tree");
		if (myDebugLevel >= DEBUG_HIGH) printTree(root);
			...
			if (bits == -1) {
				...
				if (current.myLeft == null && current.myRight == null) {
					if (current.myValue == PSEUDO_EOF) {
						if (myDebugLevel >= DEBUG_HIGH) System.out.println("PSEUDO_EOF");
						break;
					} else {
						if (myDebugLevel >= DEBUG_HIGH) System.out.println("Writing "+current.myValue);
//						if(current.myValue == 10) out.writeBits(BITS_PER_WORD, 13);
	...
	private HuffNode readTreeHeader(BitInputStream in) {
		...
		if (bit == -1)
			throw new HuffException("read -1 in readTreeHeader");
		if (bit == 0) {
			if (myDebugLevel >= DEBUG_HIGH) System.out.println("encountered inner node");
			HuffNode left = readTreeHeader(in);
			HuffNode right = readTreeHeader(in);
			return new HuffNode(0, 0, left, right);
		} else {
			int value = in.readBits(BITS_PER_WORD + 1);
			if (myDebugLevel >= DEBUG_HIGH) System.out.println("encountered leaf node with value " + value);
